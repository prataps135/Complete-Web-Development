...
app.get("/filter", (req, res, next) => {
  ...
  const foundJoke = jokes.filter((joke) => joke.jokeType === filterType);
  // jokes.find((joke)=>{
  //   if(joke.jokeType === filterQuery){
  //     foundJoke.push(joke);
  //   }
  // });
  res.json(foundJoke);
...
app.post("/jokes", (req, res, next) => {
  let newId = jokes[jokes.length - 1].id + 1;
  const newJoke = {
    id: newId,
...
app.put("/jokes/:id", (req, res, next) => {
  ...
  if (foundJoke !== null && foundJoke !== undefined) {
    ...
    res.json({ error: "Joke not found in Database." });
  }
});

//6. PATCH a joke
app.patch("/jokes/:id", (req, res, next) => {
  const id = parseInt(req.params.id);
  const foundJoke = jokes.find((joke) => joke.id === id);
  if (foundJoke !== null && foundJoke !== undefined) {
    const updatedJoke = {
      id: id,
      text: req.body.text || foundJoke.jokeText,
      type: req.body.type || foundJoke.jokeType,
    };
    const searchIndex = jokes.findIndex((joke) => joke.id === id);
    jokes[searchIndex] = updatedJoke;
    res.json(updatedJoke);
...
app.delete("/jokes/:id", (req, res, next) => {
  ...
  if (searchIndex > -1) {
    ...
    res.status(404).json({ error: "Joke not found in Database" });
  }
  next();
});
...
app.delete("/all", (req, res, next) => {
  ...
  if (key === masterKey) {
    jokes = [];
    res.sendStatus(200).json({ process: "Deleting in process" });
  } else {
